import { i18n } from '../i18n'
export const constants = {
  userTitle: [
    {
      value: 'Mr',
      id: 1
    },
    {
      value: 'Ms',
      id: 2
    },
    {
      value: 'Mrs',
      id: 3
    },
    {
      value: 'Miss',
      id: 4
    },
    {
      value: 'Dr',
      id: 5
    }
  ],
  employment: [
    {
      value: 'Student',
      id: 1
    },
    {
      value: 'Self Employed',
      id: 2
    },
    {
      value: 'Retired',
      id: 3
    },
    {
      value: 'Homemaker',
      id: 4
    },
    {
      value: 'Employed',
      id: 5
    },
    {
      value: 'Unemployed',
      id: 6
    }
  ],
  yourIncomePageData: {
    pageHeading: i18n.t('YOUR_INCOME_HEADER'),
    subHeading: i18n.t('YOUR_INCOME_LABEL'),
    incomeDescription: i18n.t('YOUR_INCOME_PAGE_DESCRIPTION'),
    incomeInfo: i18n.t('YOUR_INCOME_AMOUNT_INFO'),
    helperText: i18n.t('INCOME_HELPER_TEXT'),
    helperTextAmount: i18n.t('INCOME_HELPER_TEXT_AMOUNT')
  },
  householdIncomePageData: {
    pageHeading: i18n.t('HOUSEHOLD_INCOME_HEADER'),
    subHeading: i18n.t('HOUSEHOLD_INCOME_LABEL'),
    incomeDescription: i18n.t('HOUSEHOLD_INCOME_PAGE_DESCRIPTION'),
    incomeInfo: i18n.t('HOUSEHOLD_INCOME_AMOUNT_INFO'),
    helperText: i18n.t('INCOME_HELPER_TEXT'),
    helperTextAmount: i18n.t('INCOME_HELPER_TEXT_AMOUNT')
  },
  tenurevalue: [
    {
      value: '12 months',
      id: 1
    },
    {
      value: '13 months',
      id: 2
    },
    {
      value: '14 months',
      id: 3
    },
    {
      value: '15 months',
      id: 4
    },
    {
      value: '16 months',
      id: 5
    },
    {
      value: '17 months',
      id: 6
    },
    {
      value: '18 months',
      id: 7
    },
    {
      value: '19 months',
      id: 8
    },
    {
      value: '20 months',
      id: 9
    },
    {
      value: '21 months',
      id: 10
    },
    {
      value: '22 months',
      id: 11
    },
    {
      value: '23 months',
      id: 12
    },
    {
      value: '24 months',
      id: 13
    },
    {
      value: '25 months',
      id: 14
    },
    {
      value: '26 months',
      id: 15
    },
    {
      value: '27 months',
      id: 16
    },
    {
      value: '28 months',
      id: 17
    },
    {
      value: '29 months',
      id: 18
    },
    {
      value: '30 months',
      id: 19
    },
    {
      value: '31 months',
      id: 20
    },
    {
      value: '32 months',
      id: 21
    },
    {
      value: '33 months',
      id: 22
    },
    {
      value: '34 months',
      id: 23
    },
    {
      value: '35 months',
      id: 24
    },
    {
      value: '36 months',
      id: 25
    },
    {
      value: '37 months',
      id: 26
    },
    {
      value: '38 months',
      id: 27
    },
    {
      value: '39 months',
      id: 28
    },
    {
      value: '40 months',
      id: 29
    },
    {
      value: '41 months',
      id: 30
    },
    {
      value: '42 months',
      id: 31
    },
    {
      value: '43 months',
      id: 32
    },
    {
      value: '44 months',
      id: 33
    },
    {
      value: '45 months',
      id: 34
    },
    {
      value: '46 months',
      id: 35
    },
    {
      value: '47 months',
      id: 36
    },
    {
      value: '48 months',
      id: 37
    },
    {
      value: '49 months',
      id: 38
    },
    {
      value: '50 months',
      id: 39
    },
    {
      value: '51 months',
      id: 40
    },
    {
      value: '52 months',
      id: 41
    },
    {
      value: '53 months',
      id: 42
    },
    {
      value: '54 months',
      id: 43
    },
    {
      value: '55 months',
      id: 44
    },
    {
      value: '56 months',
      id: 45
    },
    {
      value: '57 months',
      id: 46
    },
    {
      value: '58 months',
      id: 47
    },
    {
      value: '59 months',
      id: 48
    },
    {
      value: '60 months',
      id: 49
    }
  ],
  yearsLived: [
    {
      value: '1',
      id: 1
    },
    {
      value: '2',
      id: 2
    },
    {
      value: '3',
      id: 3
    },
    {
      value: '4',
      id: 4
    },
    {
      value: '5',
      id: 5
    },
    {
      value: '6',
      id: 6
    },
    {
      value: '7',
      id: 7
    },
    {
      value: '8',
      id: 8
    },
    {
      value: '9',
      id: 9
    },
    {
      value: '10',
      id: 10
    },
    {
      value: '11',
      id: 11
    },
    {
      value: '12',
      id: 12
    },
    {
      value: '13',
      id: 13
    },
    {
      value: '14',
      id: 14
    },
    {
      value: '15',
      id: 15
    },
    {
      value: '16',
      id: 16
    },
    {
      value: '17',
      id: 17
    },
    {
      value: '18',
      id: 18
    },
    {
      value: '19',
      id: 19
    },
    {
      value: '20',
      id: 20
    },
    {
      value: '21',
      id: 21
    },
    {
      value: '22',
      id: 22
    },
    {
      value: '23',
      id: 23
    },
    {
      value: '24',
      id: 24
    },
    {
      value: '25',
      id: 25
    },
    {
      value: '26',
      id: 26
    },
    {
      value: '27',
      id: 27
    },
    {
      value: '28',
      id: 28
    },
    {
      value: '29',
      id: 29
    },
    {
      value: '30',
      id: 30
    },
    {
      value: '31',
      id: 31
    }
  ],
  residentialStatus: [
    {
      value: 'Private Tenant',
      id: 1
    },
    {
      value: 'Council Tenant',
      id: 2
    },
    {
      value: 'Homeowner',
      id: 3
    },
    {
      value: 'Living with parents',
      id: 4
    },
    {
      value: 'Others',
      id: 5
    }
  ],
  addressSuggestions: [
    {
      value: {
        addressLine1: '26 Street Name',
        addressLine2: '',
        addressLine3: '',
        city: 'Nottingham',
        postcode: 'NG32FZ'
      },
      id: 1
    },
    {
      value: {
        addressLine1: '27 Street Name',
        addressLine2: '',
        addressLine3: '',
        city: 'Nottingham',
        postcode: 'NG32FZ'
      },
      id: 2
    },
    {
      value: {
        addressLine1: '28 Street Name',
        addressLine2: '',
        addressLine3: '',
        city: 'Nottingham',
        postcode: 'NG32FZ'
      },
      id: 3
    },
    {
      value: {
        addressLine1: '29 Street Name',
        addressLine2: '',
        addressLine3: '',
        city: 'Nottingham',
        postcode: 'NG32FZ'
      },
      id: 4
    },
    {
      value: {
        addressLine1: '30 Street Name',
        addressLine2: '',
        addressLine3: '',
        city: 'Nottingham',
        postcode: 'NG32FZ'
      },
      id: 5
    },
    {
      value: {
        addressLine1: '31 Street Name',
        addressLine2: '',
        addressLine3: '',
        city: 'Nottingham',
        postcode: 'NG32FZ'
      },
      id: 6
    },
    {
      value: {
        addressLine1: '32 Street Name',
        addressLine2: '',
        addressLine3: '',
        city: 'Nottingham',
        postcode: 'NG32FZ'
      },
      id: 7
    },
    {
      value: {
        addressLine1: '33 Street Name',
        addressLine2: '',
        addressLine3: '',
        city: 'Nottingham',
        postcode: 'NG32FZ'
      },
      id: 8
    },
    {
      value: {
        addressLine1: '34 Street Name',
        addressLine2: '',
        addressLine3: '',
        city: 'Nottingham',
        postcode: 'NG32FZ'
      },
      id: 9
    }
  ],
  downloadSteps: [
    {
      value: i18n.t('STEP1_TEXT'),
      id: 1
    },
    {
      value: i18n.t('STEP2_TEXT'),
      id: 2
    },
    {
      value: i18n.t('STEP3_TEXT'),
      id: 3
    },
    {
      value: i18n.t('STEP4_TEXT'),
      id: 4
    }
  ],
  loanTypes: [
    {
      loantype: i18n.t('PERSONAL_LOANS'),
      loandesc: 'Get instant pre-approval for your personal loan',
      img: 'https://d3s87pah5oatx.cloudfront.net/uploads/8a2842e3-1b9e-48ec-96a7-dbe7ae899f30/original/personal-loans.svg',
      id: 'slide-1',
      route: '/your-name'
    },
    {
      loantype: i18n.t('MORTGAGES'),
      loandesc: 'Apply online for simple and flexible mortgages',
      img: 'https://d3s87pah5oatx.cloudfront.net/uploads/9ae1dafd-4e72-42d4-8858-3adceb187d38/original/Mortgage-img.svg',
      id: 'slide-2',
      route: ''
    },
    {
      loantype: i18n.t('CURRENT_ACCOUNTS'),
      loandesc: 'Instantly open a current account with a virtual debit card',
      img: 'https://d3s87pah5oatx.cloudfront.net/uploads/5a4e4d6c-6085-48dc-be5e-5fcc3d20fdf9/original/current-accounts.svg',
      id: 'slide-3',
      route: ''
    }
  ],
  accountTypes: [
    {
      accountType: i18n.t('CURRENT_ACCOUNTS'),
      id: 1,
      accountlogo: 'https://d3s87pah5oatx.cloudfront.net/uploads/b0e8d9f1-a358-4da4-9dff-40e77055f8b8/original/capone-current-accounts.svg',
      route: ''
    },
    {
      accountType: i18n.t('SAVINGS_ACCOUNTS'),
      id: 2,
      accountlogo: 'https://d3s87pah5oatx.cloudfront.net/uploads/d22e7ba3-cb1e-404b-98ca-951de67b7481/original/capone-Savings.svg',
      route: ''
    },
    {
      accountType: i18n.t('LOANS'),
      id: 3,
      accountlogo: 'https://d3s87pah5oatx.cloudfront.net/uploads/1643c273-561f-4a2c-86a2-c35a35cff7ab/original/capone-Loan.svg',
      route: '/your-name'
    },
    {
      accountType: i18n.t('MORTGAGES'),
      id: 4,
      accountlogo: 'https://d3s87pah5oatx.cloudfront.net/uploads/8dd8383b-7368-48bf-be39-61cb9695817e/original/capone-Mortgage.svg',
      route: ''
    },
    {
      accountType: i18n.t('CREDIT_CARDS'),
      id: 5,
      accountlogo: 'https://d3s87pah5oatx.cloudfront.net/uploads/9b05bd09-b10e-44e1-a842-2e226e98379f/original/capone-Credit_card.svg',
      route: ''
    }
  ],
  companyProfile: [
    {
      id: 1,
      option: i18n.t('ABOUT_US'),
      optionlogo: 'https://d3s87pah5oatx.cloudfront.net/uploads/49fd00ce-2545-4fd9-84d2-cd5d03ebbbe3/original/capone-About_us.svg',
      desktopLogo: 'https://d3s87pah5oatx.cloudfront.net/uploads/418f7877-4a44-4ec8-9c3c-f6b123946d35/original/Group_48097106.svg'
    },
    {
      id: 2,
      option: i18n.t('CAREERS'),
      optionlogo: 'https://d3s87pah5oatx.cloudfront.net/uploads/4c67aa0d-379f-469f-b55c-d31fa8b31dfc/original/capone-Career.svg',
      desktopLogo: 'https://d3s87pah5oatx.cloudfront.net/uploads/fca8e8cf-bbab-4af4-9b03-647d9a165b9f/original/Group_48097097_2_.svg'
    },
    {
      id: 3,
      option: i18n.t('HELP'),
      optionlogo: 'https://d3s87pah5oatx.cloudfront.net/uploads/56b400a4-6398-4581-afb5-e40f345f1c36/original/capone-Help.svg',
      desktopLogo: 'https://d3s87pah5oatx.cloudfront.net/uploads/f5329f07-355d-4dab-bedd-5472c13f815b/original/Help.svg'
    }
  ],
  approvedImages: {
    'Home improvement': { mobileImage: 'https://d3s87pah5oatx.cloudfront.net/uploads/7a8ee20e-a4a2-4d57-93f2-426d8d619917/original/home-approved-mobile-capone.svg', desktopImage: 'https://d3s87pah5oatx.cloudfront.net/uploads/c619de88-007e-45fb-a822-c721d0b894e9/original/Group_48097262.svg' },
    'Used vehicle': { mobileImage: 'https://d3s87pah5oatx.cloudfront.net/uploads/2ee37287-4126-4c94-86cc-0c7ce5415f19/original/used-vehicle-capone-mobile.svg', desktopImage: 'https://d3s87pah5oatx.cloudfront.net/uploads/00eda964-aa71-4e93-bf21-1a4f39c64ab2/original/Car_Loan_2_.svg' },
    Holiday: { mobileImage: 'https://d3s87pah5oatx.cloudfront.net/uploads/e9a6ed95-2f54-4e6e-9990-76ad1c84e528/original/holiday-approved-capone.svg', desktopImage: 'https://d3s87pah5oatx.cloudfront.net/uploads/616af254-7a74-48e7-bc4e-56f4c22720a6/original/Holidays.svg' },
    'Rainy day fund': { mobileImage: 'https://d3s87pah5oatx.cloudfront.net/uploads/03514e6b-9e99-4900-84a9-3d8807343071/original/rainy-day-mobile-capone.svg', desktopImage: 'https://d3s87pah5oatx.cloudfront.net/uploads/115ea5dc-0d87-40d0-a7e5-4bbfd1b48e07/original/Rainy_Day.svg' }
  },
  approvedText: {
    'Home improvement': i18n.t('HOME_LOAN_APPLIED_TEXT'),
    'Used vehicle': i18n.t('VECHICLE_LOAN_APPLIED_TEXT'),
    Holiday: i18n.t('HOLIDAY_LOAN_APPLIED_TEXT'),
    'Rainy day fund': i18n.t('RAINY_DAY_LOAN_APPLIED_TEXT')
  },
  dependants: [
    {
      value: '0',
      id: 1
    },
    {
      value: '1',
      id: 2
    },
    {
      value: '2',
      id: 3
    },
    {
      value: '3',
      id: 4
    },
    {
      value: '4',
      id: 5
    },
    {
      value: '5+',
      id: 6
    }
  ],
  loanPurpose: {
    'Home improvement': {
      loanType: 'Home improvement',
      id: 1,
      img: 'https://d3s87pah5oatx.cloudfront.net/uploads/f7815cb6-e42b-4aa6-8e7e-83406ac17728/original/capone-home-improvement.svg'
    },
    'Used vehicle': {
      loanType: 'Used vehicle',
      id: 2,
      img: 'https://d3s87pah5oatx.cloudfront.net/uploads/83f71443-c8e8-4ddd-8849-3710bb372e64/original/capone-used-vehicle.svg'
    },
    Holiday: {
      loanType: 'Holiday',
      id: 3,
      img: 'https://d3s87pah5oatx.cloudfront.net/uploads/170600fc-27d7-4260-89ce-3ad40746cdf7/original/capone-holiday.svg'
    },
    'Rainy day fund': {
      loanType: 'Rainy day fund',
      id: 4,
      img: 'https://d3s87pah5oatx.cloudfront.net/uploads/6352746d-7a39-45ad-880b-110e07954e04/original/capone-rainy-day-fund.svg'
    }
  },
  loanStatus: {
    rejected: {
      image: 'https://d3s87pah5oatx.cloudfront.net/uploads/3e4512a3-4f5a-4010-82aa-2a2809fff8cc/original/Group_48097095_1_.svg',
      title: i18n.t('SORRY_NO_TIME'),
      description1: i18n.t('REJECTED_DESC'),
      description2: i18n.t('REJECTED_GUIDLINES'),
      links: 'https://www.moneyhelper.org.uk/en/everyday-money/credit-and-purchases/what-to-do-if-you-have-been-refused-a-loan-or-credit-card',
      linkText: i18n.t('GUIDLINES_LINK_LABEL')
    },
    pending: {
      image: 'https://d3s87pah5oatx.cloudfront.net/uploads/937a1924-3c4a-410c-b53b-4194ea07b255/original/Group_48097094.svg',
      title: i18n.t('PROCESSING'),
      description1: i18n.t('PENDING_DESC')
    }
  }
}
